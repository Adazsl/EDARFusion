...
"""数据归一化函数，将张量缩放到【0，1】范围内
"""
def min_max(data):
    min = torch.min(data)
    max = torch.max(data)
    data = (data - min) / (max - min)#线性归一化
    return data

class Transformer2D(nn.Module):
    """2D图像变形模块，使用流场(flow field)对图像进行非线性变形"""
    def __init__(self):
    ...
    def forward(self, src, flow, padding_mode="border"):
        """使用流场对源图像进行变形
              参数:
                  src (torch.Tensor): 源图像 [B, C, H, W]
                  flow (torch.Tensor): 流场 [B, 2, H, W]
                  padding_mode (str): 边界填充模式
              返回:
                  torch.Tensor: 变形后的图像
        """
        b = flow.shape[0]# 批次大小
        size = flow.shape[2:]# 图像尺寸 (H, W)
        # 创建网格坐标
        vectors = [torch.arange(0, s) for s in size]
        grids = torch.meshgrid(vectors)# 生成网格
        grid = torch.stack(grids) # 组合网格坐标
        grid = grid.to(torch.float32)
        grid = grid.repeat(b, 1, 1, 1).to(flow.device)# 扩展到整个批次
        new_locs = grid + flow
        shape = flow.shape[2:]
        # 标准化坐标到[-1,1]范围
...
def create_affine_transformation_matrix(n_dims, scaling=None, rotation=None, shearing=None, translation=None):
    ...
    if n_dims == 2:
        if rotation is None:
            ...
            rotation = np.asarray(rotation) * (math.pi / 180)# 角度转弧度
        # 创建旋转矩阵
...
def non_affine(imgs, padding_modes, opt, elastic_random=None):
    """
       应用非仿射（弹性）变形到图像
       参数:
           imgs (list/tensor): 输入图像列表或单个图像
           padding_modes (list/str): 填充模式列表或单个模式
           opt (dict): 配置参数
           elastic_random (np.array): 预生成的随机场
       返回:
           torch.Tensor/list: 变形后的图像
       """
...
def affine(random_numbers, imgs, padding_modes, opt):
    """
       应用仿射变换到图像
       参数:
           random_numbers (np.array): 随机数数组
           imgs (list/tensor): 输入图像列表或单个图像
           padding_modes (list/str): 填充模式列表或单个模式
           opt (dict): 配置参数
       返回:
           torch.Tensor/list: 变换后的图像
       """
    # 处理输入参数
    if not isinstance(imgs, list) and not isinstance(imgs, tuple):
        imgs = [imgs]
...
def random_warp_data_list(data_list, padding_batch=False):
    """
        对数据列表中的图像应用随机变形（仿射+非仿射）

        参数:
            data_list (list): 图像列表 [img1, img2]
            padding_batch (bool): 是否添加批次维度
    ...
    opt = {
        ...
        'rotation': 3,       # 最大旋转角度(度)
        'translation': 0.08,   # 最大平移比例
        'scaling': 0.08,     # 最大缩放比例
        # non affine
        'non_affine_alpha': 120,# 非仿射变形强度
        'gaussian_smoothing': 12,# 高斯平滑核大小
    }

    random_numbers = torch.rand(8).numpy() * 2 - 1# 生成随机数用于控制变形参数
...
def get_visualize_image_pair(index1, index2, modal):
    ...
    if modal == 'CT':
        ...
        model_img1 = model_img1.unsqueeze(0)
        model_img2 = model_img2.permute(2, 0, 1)# [H,W,C] -> [C,H,W]
