...
import kornia.geometry.transform as KGT#几何变换
import kornia.utils as KU#工具函数

class Warper2d(nn.Module):
    def __init__(self):
        super(Warper2d, self).__init__()
        """
           2D图像变形模块（使用grid_sample实现）
           功能：根据流场(flow field)对图像进行变形
           原理：
           1. 创建基础网格坐标系
           2. 将流场加到基础网格上得到变形后的网格
           3. 使用grid_sample函数对图像进行采样
           优点：利用PyTorch原生函数，高效且支持GPU加速
        """
...
class warp2D:
    """
      ...
      3. 适用于需要精确控制插值行为的场景
      """
    def __init__(self, padding=False):#padding (bool): 是否在变形前填充图像边界
    ...
    def _interpolate(self, im, x, y):# 双线性插值实现，返回插值后的图像
        device = im.device
        if self.padding:
            im = F.pad(im, (1, 1, 1, 1))
