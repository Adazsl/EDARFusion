...
class RegDataset_F(Dataset):#训练数据集
    """医学图像配准训练/验证数据集类
     功能：
     - 加载配对的医学图像（如MRI-CT、MRI-PET）
     - 应用数据增强
     - 生成形变场和变形图像
     - 预处理和归一化
     参数:
         ...
         model (str): 模态类型 (如 'CT', 'PET')
         is_normalize (bool): 是否进行归一化
     """
    def __init__(self, root, mode, model, is_normalize=True):
        ...
        transforms.RandomHorizontalFlip(),#修改的地方 # 随机水平翻转 (50%概率)
        transforms.RandomVerticalFlip(),#修改的地方，后面加了（）  # 随机垂直翻转 (50%概率)
        transforms.RandomChoice([     # 随机选择旋转角度
    ...
    def __getitem__(self, item):
        ...
        if self.model == 'CT':
            ...
            model_img1 = model_img1.permute(2, 0, 1)# [H, W, C] -> [C, H, W]
            model_img2 = model_img2.unsqueeze(0)# [H, W] -> [1, H, W]

        if self.mode == 'train':
            seed = torch.seed()# 使用相同随机种子确保对两个模态应用相同的变换
...
class TestData(Dataset):
    ...
    @staticmethod
    def imread(path: pathlib.Path, flags=cv2.IMREAD_GRAYSCALE):#读取图像
