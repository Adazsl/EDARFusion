...
from modal_2d.RegFusion_lite import Encoder, ModelTransfer_lite, FusionNet_lite, \
    EntropyDiffRegNet_lite
...
def train(modal,
    ...
    train_dataloader = DataLoader( # 数据加载器
        ...
        pin_memory=True,
        shuffle=True,
        num_workers=64)# 数据加载线程数

    # 3. 模型构建（四组件架构）
    """create network"""
    encoder = Encoder().to(device)# 特征编码器
    transfer = ModelTransfer_lite(num_vit=2, num_heads=4, img_size=[256, 256]).to(device)# 模态转换器
    reg_net = EntropyDiffRegNet_lite(
        entropy_bins=16, entropy_win=9, entropy_sigma=0.08,
        tau=0.25, w_beta=1.0
    ).to(device)# 配准网络（生成位移场）
    fusion_net = FusionNet_lite().to(device)# 融合网络（多模态融合）
    for par in transfer.modal_dis.parameters():  # 冻结模态判别器参数（GAN训练技巧）
    ...
    # 5. 训练恢复逻辑
    if resume:
        checkpoint = torch.load(os.path.join('./checkpoint/checkpoint', 'checkpoint_123_CT.pkl'), map_location=torch.device('cpu'))  # load文件
        encoder.load_state_dict(checkpoint['encoder_state_dict'])# 加载各组件状态
    ...
    for iter_num in range(start_epoch, num_epoch):
        ...
        for step, batch in enumerate(epoch_iterator):# 带进度条的批次迭代
            step += 1
            ...
            # 7.4 图像配准
            _, _, flows, flow, _, _, W, (H1, H2, dH),R_feat = reg_net(feature1, feature2, img1, img2,return_maps=True,raw1=AU_F, raw2=BU_F, return_pre=True)
        ...
        print(
            f"[Epoch {iter_num + 1}/{num_epoch}] "
            f"cls:{cls_losses / step:.4f} | "
            f"trans:{transfer_losses / step:.4f} | "
            f"flow(s):{flow_losses / step:.4f} | "
            f"reg:{reg_losses / step:.4f} | "
            f"fusion:{fusion_losses / step:.4f} | "
            f"total:{epoch_loss / step:.4f} | "
        )
        ...
        if (iter_num + 1) % 5 ==0:
            ...
            path_checkpoint = "./checkpoint/checkpoint/checkpoint_123_CT.pkl"
    ...
    checkpoint_ = {
        ...
        "fusion_net_state_dict": fusion_net.state_dict()}
    # 13. 最终模型保存
    torch.save(checkpoint_, os.path.join(checkpoint_path, f'EDARFusion-{modal}.pkl'))
